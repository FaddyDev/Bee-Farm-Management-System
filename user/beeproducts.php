 ...
 <table class="table table-stripped table-hover table-bordered table-condensed" style="background-color:white;">
	...
	<tbody>		
		...
		require("include/connection.php");				
			$results = mysql_query("SELECT * FROM `products`") or die (mysql_error());			
			while($project = mysql_fetch_array($results)){
				...
				<tr> 
					...
					<td>".$project['owner']."</td>					
					<td> <img class='img img-responsive' src='../admin/products-uploads/".$project['image']."'  /> </td>					
					
					<td><form method='post' action='beeproducts.php'><button name='add".$project['product_id']."' type='submit' class='btn btn-success' ><span class='glyphicon glyphicon-shopping-cart'></span>  Add To Cart</button></form></td>
					
				</tr>
				";
				
				if(isset($_POST["add".$project['product_id']])){
				...
				mysql_query("INSERT INTO `cart`(`item_id`, `name`, `unitprice`, `qty`, `tatol` ,`orderby`) VALUES (null,'".$project['name']."','".$project['price']."','1','".$project['price']."' ,'".$_SESSION['loguser']."') ") or die(mysql_error());
			        ...
			        while($project1 = mysql_fetch_array($results1)){
						...
						$qty=$project1['qty']+1;
						if($qty > $project['quantity']){echo "<script> alert('can not order more than ".$project['quantity']." units of ".$project['name']." at the moment ');</script>";}else{
				...
				echo "<script> window.location='beeproducts.php'</script>";
